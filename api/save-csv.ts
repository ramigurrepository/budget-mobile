...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const [{ data: regular }, { data: recurring }] = await Promise.all([
    supabase.from("expenses").select("*").eq("household_id", hid).eq("is_active", true).eq("is_recurring", false).gte("date", startDate).lte("date", endDate),
    supabase.from("expenses").select("*").eq("household_id", hid).eq("is_active", true).eq("is_recurring", true)
  ...
  if (recurring?.length) {
    ...
    const { data: exceptions } = await supabase.from("recurring_exceptions").select("expense_id").in("expense_id", recurringIds).eq("year", year).eq("month", month)
  ...
  const lines: string[] = [`﻿דוח הוצאות — ${monthName} ${year}`, "תאריך,קטגוריה,תיאור,סכום,משלם,אמצעי תשלום,קבוע/חד-פעמי,תשלומים"]
