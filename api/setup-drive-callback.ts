...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!refreshToken) {
    return res.status(200).send(`
      ...
      <p>Try again — make sure to approve the consent screen fully.</p>
  ...
  return res.status(200).send(`
    <h2>✅ Success!</h2>
    <p>Copy this refresh token and send it to Claude:</p>
    <textarea rows="4" cols="80" onclick="this.select()">${refreshToken}</textarea>
  `)
}
